...
contract StdConfig {
    ...
    constructor(string memory configFilePath, bool writeToFile) {
        ...
        for (uint256 i = 0; i < chain_keys.length; i++) {
            ...
            if (
                vm
                    .parseTomlKeys(content, string.concat("$.", chain_key))
                    .length == 0
            ) {
                continue;
            }
            ...
            try
                vm.parseTomlString(
                    content,
                    string.concat("$.", chain_key, ".endpoint_url")
                )
            ...
            for (uint8 t = 1; t <= NUM_TYPES; t++) {
                ...
                string memory typePath = string.concat(
                    "$.",
                    chain_key,
                    ".",
                    ty.toTomlKey()
                ...
                try vm.parseTomlKeys(content, typePath) returns (
                    string[] memory keys
                ) {
    ...
    function _loadAndCacheValue(
        ...
        if (ty == TypeKind.Bool) {
            try vm.parseTomlBool(content, path) returns (bool val) {
                ...
                try vm.parseTomlBoolArray(content, path) returns (
                    bool[] memory val
                ) {
                    _dataOf[chainId][key] = abi.encode(val);
            ...
            try vm.parseTomlAddress(content, path) returns (address val) {
                ...
                try vm.parseTomlAddressArray(content, path) returns (
                    address[] memory val
                ) {
                    _dataOf[chainId][key] = abi.encode(val);
            ...
            try vm.parseTomlBytes32(content, path) returns (bytes32 val) {
                ...
                try vm.parseTomlBytes32Array(content, path) returns (
                    bytes32[] memory val
                ) {
                    _dataOf[chainId][key] = abi.encode(val);
            ...
            try vm.parseTomlUint(content, path) returns (uint256 val) {
                ...
                try vm.parseTomlUintArray(content, path) returns (
                    uint256[] memory val
                ) {
                    _dataOf[chainId][key] = abi.encode(val);
            ...
            try vm.parseTomlInt(content, path) returns (int256 val) {
                ...
                try vm.parseTomlIntArray(content, path) returns (
                    int256[] memory val
                ) {
                    _dataOf[chainId][key] = abi.encode(val);
            ...
            try vm.parseTomlBytes(content, path) returns (bytes memory val) {
                ...
                try vm.parseTomlBytesArray(content, path) returns (
                    bytes[] memory val
                ) {
                    _dataOf[chainId][key] = abi.encode(val);
            ...
            try vm.parseTomlString(content, path) returns (string memory val) {
                ...
                try vm.parseTomlStringArray(content, path) returns (
                    string[] memory val
                ) {
                    _dataOf[chainId][key] = abi.encode(val);
    ...
    function _ensureTypeConsistency(
        uint256 chainId,
        string memory key,
        Type memory ty
    ) private {
    ...
    function get(
        uint256 chain_id,
        string memory key
    ) public view returns (Variable memory) {
    ...
    function set(uint256 chainId, string memory key, bool value) public {
        ...
        if (_writeToFile)
            _writeToToml(chainId, ty.kind.toTomlKey(), key, vm.toString(value));
    }
    ...
    function set(
        ...
        _dataOf[chainId][key] = abi.encode(value);
        if (_writeToFile)
            _writeToToml(chainId, ty.kind.toTomlKey(), key, _quote(value));
    }

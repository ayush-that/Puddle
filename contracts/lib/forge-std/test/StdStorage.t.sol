...
contract StdStorageTest is Test {
    ...
    function test_StorageExtraSload() public {
        assertEq(
            16,
            stdstore.target(address(test)).sig(test.extra_sload.selector).find()
        );
    }
    ...
    function test_StorageCheckedWriteSignedIntegerObvious() public {
        stdstore.target(address(test)).sig(test.tG.selector).checked_write_int(
            -100
        );
    ...
    function testFuzz_StorageCheckedWriteMapPacked(
        address addr,
        uint128 value
    ) public {
        stdstore
    ...
    function testFuzz_StorageNativePack(
        ...
        stdstore
            .enable_packed_slots()
            .target(address(test))
            .sig(test.tA.selector)
            .checked_write(val1);
        stdstore
            .enable_packed_slots()
            .target(address(test))
            .sig(test.tB.selector)
            .checked_write(boolVal1);
        stdstore
            .enable_packed_slots()
            .target(address(test))
            .sig(test.tC.selector)
            .checked_write(boolVal2);
        stdstore
            .enable_packed_slots()
            .target(address(test))
            .sig(test.tD.selector)
            .checked_write(val2);
    ...
    function testFuzz_Packed(uint256 val, uint8 elemToGet) public {
        ...
        for (uint256 i = 1; i < 5; i++) {
            ...
            uint256 expectedValToRead = (val << leftBits) >>
                (leftBits + rightBits);
    ...
    function testFuzz_Packed2(uint256 nvars, uint256 seed) public {
        ...
        for (uint256 i = 0; i < nvars; i++) {
            ...
            sizes[i] = bound(
                uint256(keccak256(abi.encodePacked(seed, i + 256))),
                1,
                maxVarSize
            );
